...
class CAMComputer(object):
    def __init__(self,
        ...
        self.top1: list = None
        self.top5: list = None
        self.per_cl_perf: dict = None

    @staticmethod
    def recreate_dataloader(dataset):
        shuffle = dataset._backup_shuffle
        batch_size = dataset._backup_batch_size
        workers = dataset._backup_num_workers
        collate_fn = dataset._backup_collate_fn
        use_ds = dataset._backup_use_distributed_sampler
        sampler = None
        if use_ds:
    ...
    def create_evaluator(self):
        ...
        evaluator = None  # not needed for trainset.
        if self.split != constants.TRAINSET:
            evaluator = {
                constants.OpenImages: MaskEvaluator,
                constants.CUB: BoxEvaluator,
                constants.ILSVRC: BoxEvaluator,
                constants.YTOV1: BoxEvaluator,
                constants.YTOV22: BoxEvaluator
            }[self.dataset_name](metadata=self.metadata,
    ...
    def _build_colocam_extractor(self, model, args):
    ...
    def get_cam_one_sample(self,
        ...
        if task == constants.STD_CL:
        ...
        elif task in [constants.F_CL, constants.TCAM, constants.COLOCAM]:
            ...
            elif task == constants.COLOCAM:
                cam = self.colocam_extractor(argmax=self.fcam_argmax)
                # (h`, w`)
    ...
    def compute_and_evaluate_cams(self):
        ...
        unique_cls: set = self.loader.dataset.unique_cl_labels
        # average performance over classes.
        max_box_acc: list = None
        top1 = []
        top5 = []

        per_cl_perf = dict()  # per class performance.

        dataset = copy.deepcopy(self.loader.dataset)
        dataset._switcher_turn_back_original_img_ids()
        self.loader = self.recreate_dataloader(dataset)


        for cl in unique_cls:
            print(f"Computing and evaluating cams for class: {cl}")
            self.evaluator = self.create_evaluator()

            dataset._switcher_show_only_class_k(k=cl)
            self.loader = self.recreate_dataloader(dataset)


            stats = self._single_class_compute_and_evaluate_cams()

            if max_box_acc is None:
                max_box_acc = stats
                top1 = copy.deepcopy(self.evaluator.top1)
                top5 = copy.deepcopy(self.evaluator.top5)

            else:
                max_box_acc = self.sum_2_lists_element_wise(stats, max_box_acc)
                top1 = self.sum_2_lists_element_wise(top1, self.evaluator.top1)
                top5 = self.sum_2_lists_element_wise(top5, self.evaluator.top5)

            per_cl_perf[cl] = {
                'iou_thresholds': copy.deepcopy(self.iou_threshold_list),
                # list
                constants.LOCALIZATION_MTR: self.to_float(
                    copy.deepcopy(stats)),  # list
                'top1_loc': self.to_float(
                    copy.deepcopy(self.evaluator.top1)),  # list
                'top5_loc': self.to_float(
                    copy.deepcopy(self.evaluator.top5)),  # list
                'best_tau_list': copy.deepcopy(self.evaluator.best_tau_list),
                # list
                'iou_threshold_list': copy.deepcopy(
                    self.evaluator.iou_threshold_list)  # list
            }
        ...
        # avg
        n_cl = len(unique_cls)
        max_box_acc = [m / float(n_cl) for m in max_box_acc]
        top1 = [m / float(n_cl) for m in top1]
        top5 = [m / float(n_cl) for m in top5]

        self.top1 = top1
        self.top5 = top5
        self.per_cl_perf = per_cl_perf

        return max_box_acc

    @staticmethod
    def to_float(l: list) -> list:
        return [float(v) for v in l]

    def sum_2_lists_element_wise(self, l1: list, l2: list) -> list:
        assert isinstance(l1, list), type(l1)
        assert isinstance(l2, list), type(l2)
        assert len(l1) == len(l2), f'{len(l1)} {len(l2)}'

        return [m1 + m2 for m1, m2 in zip(l1, l2)]


    def _single_class_compute_and_evaluate_cams(self):
    ...
    def _draw_some_best_pred(self,
                             ...
                             per_class_best_tau_list_: dict = None,
        ...
        for _image_id in tqdm(ids_to_draw, ncols=constants.NCOLS, total=len(
            ...
            # tau per class.
            if best_tau_list_ is None and per_class_best_tau_list_ is None:
                assert self.per_cl_perf[target]['best_tau_list'] != []
                best_tau_list = self.per_cl_perf[target]['best_tau_list']

            elif best_tau_list_ is None and per_class_best_tau_list_ is not \
                    None:
                best_tau_list = per_class_best_tau_list_[target]
            else:
            ...
            if isinstance(self.evaluator, BoxEvaluator) or self.evaluator is \
                ...
                for k, _THRESHOLD in enumerate(iou_threshold_list):
                    ...
                    # todo: delete. only WSOL-VIDEO
                    if _THRESHOLD != iou_thresh:
                        continue
