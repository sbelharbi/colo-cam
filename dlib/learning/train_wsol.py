...
from dlib.cams.tcam_seeding import CoLoCAMSeeder
...
from dlib.cams import build_colocam_extractor
...
class Trainer(Basic):
    ...
    def __init__(self,
        ...
        # self.loaders[constants.TRAINSET].dataset._stats_n_frames_per_shot()
        # loader = self.loaders[constants.TRAINSET]
        # lx = []
        # for batch_idx, (
        #         images, targets, images_id, raw_imgs, std_cams,
        #         seq_iter, frm_iter, roi) in tqdm(
        #     enumerate(loader), ncols=constants.NCOLS,
        #     total=len(loader), desc=f'BUCKET '):
        #     print(images.shape)
        #     lx.append(images.shape[0])
        #
        # with open(f'stats-sizes-minibatch-{args.sample_n_from_seq}.pkl', 'wb') as fxx:
        #     pkl.dump(lx, fxx, protocol=pkl.HIGHEST_PROTOCOL)
        # sys.exit()
        ...
        if args.task == constants.COLOCAM:
            self.sl_mask_builder: CoLoCAMSeeder = self._get_sl(args)
        ...
        self.colocam_extractor = None
        if args.task == constants.COLOCAM:
            self.colocam_extractor = self._build_colocam_extractor(
                model=self._pytorch_model, args=self.args)
    ...
    @staticmethod
    def _build_colocam_extractor(model, args):
        return build_colocam_extractor(model=model, args=args)
    ...
    def _get_sl(args):
        if args.task == constants.F_CL:
            ...
            return CoLoCAMSeeder(
                seed_tech=args.sl_clc_seed_tech,
                min_=args.sl_clc_min,
                max_=args.sl_clc_max,
                ksz=args.sl_clc_ksz,
                max_p=args.sl_clc_max_p,
                min_p=args.sl_clc_min_p,
                fg_erode_k=args.sl_clc_fg_erode_k,
                fg_erode_iter=args.sl_clc_fg_erode_iter,
                support_background=args.model['support_background'],
                multi_label_flag=args.multi_label_flag,
                seg_ignore_idx=args.seg_ignore_idx,
                cuda_id=args.c_cudaid,
    ...
    def is_seed_required(self, _epoch: int) -> bool:
        if self.args.task == constants.F_CL:
            ...
            cmd = (self.args.task == constants.COLOCAM)
            cmd &= self.args.sl_clc
            cmd &= ('self_learning_co_lo_cam' in self.loss.n_holder)
            cmd2 = False
            for _l in self.loss.losses:
                if isinstance(_l, losses.SelfLearningCoLoCam):
                    cmd2 = _l.is_on(_epoch=_epoch)
    ...
    def is_compute_stats_pred_required(self, _epoch: int) -> bool:
        ...
        if self.args.task == constants.COLOCAM:
            cmd = (self.args.task == constants.COLOCAM)
            cmd &= self.args.sizefg_tmp_clc
            cmd &= ('fg_size_co_lo_cam' in self.loss.n_holder)
            cmd2 = False
            for _l in self.loss.losses:
                if isinstance(_l, losses.FgSizeCoLoCam):
                    cmd2 = _l.is_on(_epoch=_epoch)
    ...
    def _pre_forward(self, output, images: torch.Tensor,
        ...
        if self.args.task == constants.STD_CL:
            ...
            if self.is_compute_stats_pred_required(self.epoch):
                ...
                _raw_cams = deepcopy(self.colocam_extractor.model.cams)
                area = []
                total = 0.0

                for i in range(_raw_cams.shape[0]):

                    self.colocam_extractor.model.cams = _raw_cams[
                        i].unsqueeze(0)
                    _built_cam = self.colocam_extractor().detach()  # h, w
                    # stats.
                    total = _built_cam.shape[0] * _built_cam.shape[1]
                    area.append(_built_cam.sum())
    ...
    def _wsol_training(self,
        ...
        if self.args.task == constants.STD_CL:
            ...
            if self.is_seed_required(_epoch=self.epoch):
                ...
                if _cnd:  # todo: stop loading classifier's cams in dataset.
                    ...
                    _getter_roi = GetRoiSingleCam(
                        roi_method=self.args.sl_tc_roi_method,
            ...
            if self.is_seed_required(_epoch=self.epoch):
                ...
                tt = self.args.sl_clc_epoch_switch_to_sl
                _cnd = (tt != -1)
                _cnd &= (self.epoch >= tt)
                t2 = self.args.empty_out_bb_clc_start_ep
                _cnd |= self.args.empty_out_bb_clc and (self.epoch >= t2)
                ...
                if _cnd:  # todo: stop loading classifier's cams in dataset.
                    with torch.no_grad():  # todo: check grad/no grad.
                        self.best_model_loc.eval()
                        colocam_extractor = self._build_colocam_extractor(
                            model=self.best_model_loc, args=self.args)
                        _out_best_model = self.best_model_loc(images)

                    _raw_cams = deepcopy(colocam_extractor.model.cams)

                    # overwrite cams_inter
                    cams_inter = None

                    for  i in range(_raw_cams.shape[0]):

                        colocam_extractor.model.cams = _raw_cams[i].unsqueeze(0)
                        _built_cam = colocam_extractor().detach()  # h, w
                        _built_cam = _built_cam.unsqueeze(0).unsqueeze(0)
                    ...
                    _msk_bbox = None
                    _getter_roi = GetRoiSingleCam(
                        roi_method=self.args.sl_clc_roi_method,
                        p_min_area_roi=self.args.sl_clc_roi_min_size
                    )
                    _cams_cpu = cams_inter.detach().cpu()
                    ksz = constants.KEY_CAM_FG_SZ
                    fg_size = []
                    ...
                    roi: torch.Tensor = msk_bbox.long()
    ...
    def on_epoch_start(self):
        ...
        if self.args.task in [constants.TCAM, constants.COLOCAM]:
            seed_tech = self.loaders[
                constants.TRAINSET].dataset.tmp_manager.seed_tech
    ...
    @staticmethod
    def chunk_index_block_into_minibatches(block_sz: int,
                                           batch_sz: int) -> List:
        out = []
        l = list(range(block_sz))
        for z in chunk_it(l, batch_sz):
            _start = z[0]
            _end = z[-1]
    ...
    def train(self, split, epoch):
        ...
        for bucket in range(nbr_tr_bucket):
            ...
            for batch_idx, (_images,
                            ...
                            _seq_iter,
                            _frm_iter,
                            _roi) in tqdm(enumerate(loader),
                                          ncols=constants.NCOLS,
                                          total=len(loader),
                                          desc=f'BUCKET {bucket}'
                                               f'/{nbr_tr_bucket}'):


                block_size = _images.shape[0]
                true_batch_sz = self.args.batch_size_backup

                # the sampled minibatch could be larger than the requested
                # minibatch size (e.g. in colo-cam method.). so we are going
                # to split this sampled block into mini-blocks that have the
                # true size of the requested minibatch.

                n_updates = 0
                for bornes in self.chunk_index_block_into_minibatches(
                        block_size, true_batch_sz):
                    _s = bornes[0]
                    _e = bornes[1] + 1
                    _sz = _e - _s
                    ...
                    _cnd = (n_updates > 0)
                    _cnd &= (_sz < true_batch_sz)
                    _cnd &= self.args.drop_small_tr_batch

                    if _cnd:
                        # todo: control the output of this func when we dont
                        #  do any updates.
                        continue  #  or break the loop (must be the last one)

                    # split elements. ++++++++++++++++++++++++++++++++++++++++++
                    images = _images[_s : _e]
                    targets = _targets[_s : _e]
                    raw_imgs = _raw_imgs[_s : _e]
                    images_id = _images_id[_s : _e]
                    seq_iter = _seq_iter[_s : _e]
                    frm_iter = _frm_iter[_s : _e]
                    ...
                    if _std_cams.ndim == 1:
                        ...
                        assert _std_cams.ndim == 4
                        std_cams = _std_cams[_s : _e]
                    ...
                    # images_id: tuple.
                    images_id: list = list(images_id)  # tuple to list.
                    ...
                    with autocast(enabled=self.args.amp):
                        logits, loss = self._wsol_training(images_id,
                                                           images,
                                                           raw_imgs,
                                                           targets,
                                                           std_cams,
                    ...
                    with torch.no_grad():
                        ...
                        if total_loss is None:
                            total_loss = loss.detach().squeeze() \
                                         * images.size(0)
                        else:
                            total_loss += loss.detach().squeeze() \
                                          * images.size(0)
                    ...
                    if cnd:
                        ...
                        # todo: write function for all models. STD_CL:OK.
    ...
    def evaluate(self, epoch, split, checkpoint_type=None, fcam_argmax=False):
        ...
        if self.args.dataset in [constants.CUB,
            ...
            for idx, IOU_THRESHOLD in enumerate(self.args.iou_threshold_list):
                ...
                self.performance_meters[split][
                    ...
                    cam_computer.top1[idx])

                self.performance_meters[split][
                    'top5_loc_{}'.format(IOU_THRESHOLD)].update(
                    cam_computer.top5[idx])

            if split == constants.TESTSET and self.args.is_master:
                # todo: no longer support these plots. we evaluate per-class.
                pass
        ...
        if split == constants.TESTSET and self.args.is_master:
            ...
            with open(join(outd, f'thresholds-{checkpoint_type}.yaml'),
                      'w') as fth:
                yaml.dump(cam_computer.per_cl_perf, fth)
                # yaml.dump({
                #     'iou_threshold_list':
