...
class ResNetEncoder(ResNet, EncoderMixin):
    def __init__(self, out_channels, depth=5, **kwargs):
        ...
        self.name = 'null-name'
        self.task = None
        self.apply_self_attention = False
        self.encoder_self_attention: list = []
        self.temperature = 1.
    ...
    def get_self_attention(self, features: torch.Tensor
                           ) -> Tuple[torch.Tensor, torch.Tensor]:
        assert features.ndim == 4, features.ndim  # b, d, h, w
        x = features

        raw_att = x.mean(dim=1, keepdim=True)  # b, 1, h, w
        att = torch.sigmoid(raw_att * self.temperature)
    ...
    def forward(self, x):
        ...
        for i in range(self._depth + 1):
            x = stages[i](x)  # b, d, h, w

            att, raw_att = self.get_self_attention(x)  # b, 1, h, w
            self.encoder_self_attention.append(raw_att)  # todo: store when
            # necessary.
