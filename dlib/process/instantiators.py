...
def get_loss_std_cl(args):
    ...
    cl_loss = losses.ClLoss(cuda_id=args.c_cudaid,
                            support_background=support_background,
                            multi_label_flag=multi_label_flag)
    if args.std_label_smooth:
        cl_loss.set_it(label_smoothing_e=args.std_label_smooth_e)
    else:
        assert args.std_label_smooth_e == 0.0, args.std_label_smooth_e

        cl_loss.set_it(label_smoothing_e=0.0)
    masterloss.add(cl_loss)

    if args.crf_std:
        crf_std_loss = losses.ConRanFieldStdAtt(
            cuda_id=args.c_cudaid,
            lambda_=args.crf_std_lambda,
            sigma_rgb=args.crf_std_sigma_rgb,
            sigma_xy=args.crf_std_sigma_xy,
            scale_factor=args.crf_std_scale,
            multi_label_flag=multi_label_flag,
            start_epoch=args.crf_std_start_ep,
            end_epoch=args.crf_std_end_ep)

        crf_std_loss.set_it(std_layers_att=args.std_layers_att,
                            heat=args.crf_std_tmp)

        masterloss.add(crf_std_loss)

    if args.rgb_jcrf_std:
        crf_rgb_loss = losses.RgbJointConRanFieldStdAtt(
            cuda_id=args.c_cudaid,
            lambda_=args.rgb_jcrf_std_lambda,
            sigma_rgb=args.rgb_jcrf_std_sigma_rgb,
            scale_factor=args.rgb_jcrf_std_scale,
            multi_label_flag=multi_label_flag,
            start_epoch=args.rgb_jcrf_std_start_ep,
            end_epoch=args.rgb_jcrf_std_end_ep)

        crf_rgb_loss.set_it(std_layers_att=args.std_layers_att,
                            heat=args.rgb_jcrf_std_tmp)
        masterloss.add(crf_rgb_loss)
...
def get_loss_coloc_cam(args):
    masterloss = losses.MasterLoss(cuda_id=args.c_cudaid)
    support_background = args.model['support_background']
    multi_label_flag = args.multi_label_flag
    assert not multi_label_flag

    assert args.task == constants.COLOCAM
    ...
    if args.crf_clc:
        masterloss.add(losses.ConRanFieldCoLoCam(
            cuda_id=args.c_cudaid,
            lambda_=args.crf_clc_lambda,
            sigma_rgb=args.crf_clc_sigma_rgb,
            sigma_xy=args.crf_clc_sigma_xy,
            scale_factor=args.crf_clc_scale,
            support_background=support_background,
            multi_label_flag=multi_label_flag,
            start_epoch=args.crf_clc_start_ep,
            end_epoch=args.crf_clc_end_ep,
        ))

    if args.rgb_jcrf_clc:
        rgb_loss = losses.RgbJointConRanFieldCoLoCam(
            cuda_id=args.c_cudaid,
            lambda_=args.rgb_jcrf_clc_lambda,
            sigma_rgb=args.rgb_jcrf_clc_sigma_rgb,
            scale_factor=args.rgb_jcrf_clc_scale,
            support_background=support_background,
            multi_label_flag=multi_label_flag,
            start_epoch=args.rgb_jcrf_clc_start_ep,
            end_epoch=args.rgb_jcrf_clc_end_ep,
        )
        rgb_loss.set_dimension_reduction(data_type=args.rgb_jcrf_clc_input_data,
                                         re_dim=args.rgb_jcrf_clc_input_re_dim)
        rgb_loss.set_weight_style(weight_style=args.rgb_jcrf_clc_lambda_style)
        masterloss.add(rgb_loss)

    if args.max_sizepos_clc:
        masterloss.add(losses.MaxSizePositiveCoLoCam(
            cuda_id=args.c_cudaid,
            lambda_=args.max_sizepos_clc_lambda,
            elb=deepcopy(elb),
            support_background=support_background,
            multi_label_flag=multi_label_flag,
            start_epoch=args.max_sizepos_clc_start_ep,
            end_epoch=args.max_sizepos_clc_end_ep
        ))

    if args.min_entropy_clc:
        masterloss.add(losses.EntropyCoLoCam(
            cuda_id=args.c_cudaid,
            lambda_=args.min_entropy_clc_lambda,
            support_background=support_background,
            multi_label_flag=multi_label_flag,
            start_epoch=args.min_entropy_clc_start_ep,
            end_epoch=args.min_entropy_clc_end_ep
        ))


    if args.sizefg_tmp_clc:
        _loss_fg_sz = losses.FgSizeCoLoCam(
            cuda_id=args.c_cudaid,
            lambda_=args.sizefg_tmp_clc_lambda,
            elb=deepcopy(elb),
            support_background=support_background,
            multi_label_flag=multi_label_flag,
            start_epoch=args.sizefg_tmp_clc_start_ep,
            end_epoch=args.sizefg_tmp_clc_end_ep
        )
        _loss_fg_sz.set_eps(eps=args.sizefg_tmp_clc_eps)
        masterloss.add(_loss_fg_sz)


    if args.size_bg_g_fg_clc:
        masterloss.add(losses.BgSizeGreatSizeFgCoLoCam(
            cuda_id=args.c_cudaid,
            lambda_=args.size_bg_g_fg_clc_lambda,
            elb=deepcopy(elb),
            support_background=support_background,
            multi_label_flag=multi_label_flag,
            start_epoch=args.size_bg_g_fg_clc_start_ep,
            end_epoch=args.size_bg_g_fg_clc_end_ep
        ))

    if args.empty_out_bb_clc:
        masterloss.add(losses.EmptyOutsideBboxCoLoCam(
            cuda_id=args.c_cudaid,
            lambda_=args.empty_out_bb_clc_lambda,
            elb=deepcopy(elb),
            support_background=support_background,
            multi_label_flag=multi_label_flag,
            start_epoch=args.empty_out_bb_clc_start_ep,
            end_epoch=args.empty_out_bb_clc_end_ep
        ))

    if args.sl_clc:
        sl_colocam = losses.SelfLearningCoLoCam(
            cuda_id=args.c_cudaid,
            lambda_=args.sl_clc_lambda,
            support_background=support_background,
            multi_label_flag=multi_label_flag,
            start_epoch=args.sl_clc_start_ep,
            end_epoch=args.sl_clc_end_ep,
            seg_ignore_idx=args.seg_ignore_idx
        )

        masterloss.add(sl_colocam)
...
def get_loss(args):
    ...
    # colocam
        ...
        masterloss = get_loss_coloc_cam(args)
...
def get_pretrainde_classifier(args, pretrained_ch_pt: str):
    ...
    apply_self_attention = args.model['cl_apply_self_attention']

    if apply_self_attention:
        z = args.std_layers_att.split('-')
        z = [int(e) for e  in z]
        for s in z:
            msg = f'{s} {constants.MAX_NBR_LAYERS[p.encoder_name]}'
            assert 1 <= s <= constants.MAX_NBR_LAYERS[p.encoder_name], msg
    ...
    model = create_model(
        ...
        apply_self_attention=apply_self_attention,
        attention_temp=args.model['layers_att_tmp']
    )
    ...
    if args.task == constants.COLOCAM:
        assert pretrained_ch_pt is not None
        assert pretrained_ch_pt == constants.BEST_LOC
        assert pretrained_ch_pt == args.colocam_pretrained_seeder_ch_pt
...
def get_model(args, eval=False):
    ...
    if eval:
        ...
        elif args.task in [constants.F_CL, constants.TCAM, constants.COLOCAM]:
            _, cpt = find_last_checkpoint(path, constants.CHP_BEST_M)
