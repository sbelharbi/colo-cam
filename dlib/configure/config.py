...
from dlib.configure import init_methods
...
def get_config(ds):
    ...
    args = {
        ...
        "model": {
            ...
            "cl_apply_self_attention": False,  # whether to use or not
            # layer-wise self-attention at the classifier. If true,
            # it is applied across all layers.
            "layers_att_tmp": 1.,  # temperature for attention within
            # net. float > 0.
        },
        # ======================================================================
        ...
        # ======================================================================
        # for ALL METHODS:
        # sampling frame limit:
        "sample_fr_limit": 1.,  # percentage in ]0, 1.]. percentage of frames
        # allow to be sampled from during training. eg. 0.5 means only 50% of
        # frames from a video are allowed to be sampled from. this 50% are
        # centered around the middle of the video == 25% at the left of the
        # middle, and the other 25% are at the right of the center. this can
        # help drop potentially noisy frames at the dge of a video. 1. means
        # sample from entire video without restriction.
        "sample_n_from_seq": 1,  # int. how many frames to sample from a
        # sequence(shot, video) at once.
        "min_tr_batch_sz": -1,  # int. the minimum train batch size. this is
        # could be helpful when sample_n_from_seq > 1 --> batch size will be
        # reduced. in this case, we ask to sample only:
        # 'batch_size' //  sample_n_from_seq = Z.
        # LATER, we will more likely need to duplicate samples in Z to
        # fill exactly the batch size ('batch_size') because sampling only
        # from Z videos will not necessarily fill 'batch_size'. to reduce this
        # scenario,
        # we can set a minimum batch size to ovoid to drop to Z samples.
        # however, we may load more than "batch_size" (MORE RAM).
        # we load a large block and keep it
        # in RAM and loop through it with true batch size ('batch_size').
        # if this value is very high, we need more RAM.
        # use wisely. when -1, we do nothing. BENIFITS: 1. avoid duplicates.
        # 2. process a number of frames from same videos <= n (requested
        # number). when duplicating, we get in the case of number of frames >
        # n. 3. more speed (no extra, unnecessary computations).
        "drop_small_tr_batch": False,  # bool. if true, batches with size
        # smaller than "batch_size" will be dropped. this is
        # related to "min_tr_batch_sz" and not the 'drop_last' of pytorch
        # dataloader. dropping is allowed only if, within the same BLOCK,
        # there was at least a full minibatch with size "batch_size".
        # meaning, if we did at elast one update from the current BLOCK of
        # all sampled samples.
        "sample_n_from_seq_dist": constants.SAMPLE_FR_UNIF,  #
        # whatever the style, if  sample_n_from_seq=1, dist shows how to
        # sample the frame.
        # if sample_n_from_seq > 1: dist shows how to sample only the first
        # frame except when style == random. if it is random, the next frames
        # will follow the same dist.
        # if dist==interval, style MUST be random. inside each interval we
        # sample uniformly.
        "sample_n_from_seq_std": 1.,  # std dev of the Gaussian dist (float>0).
        "sample_n_from_seq_style": constants.TIME_RANDOM,  # random,
        # adjacent(before, after, before-after). see constants.TIME_DEPENDENCY2.
        # ----------------------------------------------------------------------
        # STD-CL or when classifier is trained: label smoothing
        "std_label_smooth": False,  # use or not label smoothing for image cl.
        "std_label_smooth_e": 0.0,  # epsilon in [0, 1[.
        # crf over attention.
        "std_layers_att": '',  # layers to use their attention for LOSSES:
        # crf_std, rgb_jcrf_std, ... for std classifier. it is a string,
        # separated by '-'. eg. '5-4-3' to apply loss over the 5th, 4th,
        # and 3th layer. the 1st layer is the input layer. see
        # constants.MAX_NBR_LAYERS for max number of layers per arch. if this
        # is active, std classifier will activate the APPLICATION of
        # self-attention across ALL the layer of the net, and not just the
        # specified ones here. Counting layers starts from 1 (not 0).

        # crf over attention.
        "crf_std": False,  # use or not crf over attentions.  (penalty)
        "crf_std_lambda": 2.e-9,  # crf lambda
        "crf_std_sigma_rgb": 15.,
        "crf_std_sigma_xy": 100.,
        "crf_std_scale": 1.,  # scale factor for input, segm.
        "crf_std_tmp": 1.,  # temperature for sigmoid. float > 0.
        "crf_std_start_ep": 0,  # epoch when to start crf loss.
        "crf_std_end_ep": -1,  # epoch when to stop using crf loss. -1: never
        # stop.
        # rgb kernel of crf over attention.
        "rgb_jcrf_std": False,  # use or not joint crf over cams over
        # multiple images. apply only color penalty. loss attention.
        "rgb_jcrf_std_lambda": 2.e-9,  # crf lambda
        "rgb_jcrf_std_sigma_rgb": 15.,
        "rgb_jcrf_std_scale": 1.,  # scale factor for input, segm.
        "rgb_jcrf_std_tmp": 1.,  # temperature for sigmoid. float > 0.
        "rgb_jcrf_std_start_ep": 0,  # epoch when to start crf loss.
        "rgb_jcrf_std_end_ep": -1,  # epoch when to stop using crf loss.
        # -1: never stop.
        # ----------------------------------------------------------------------
    ...
    # add coloc.
    args = init_methods.add_coloc_cam(args)
