...
class STDClModel(torch.nn.Module):
    ...
    def free_mem(self):
        self.x_in = None
        if hasattr(self.classification_head, 'free_mem'):
            self.classification_head.free_mem()
...
class FCAMModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        self.decoder_out_ft = decoder_output.detach()  # b, c, h, w.
...
class TCAMModel(FCAMModel):
    pass


class CoLoCAMModel(FCAMModel):
    pass
...
class BoxModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        maps = self.segmentation_head(decoder_output)

        if maps.shape[2:] != x_shape[2:]:
            maps = F.interpolate(
                input=maps,
                size=x_shape[2:],
                mode='bilinear',
        ...
        self.maps = maps.detach()

        return maps

    def train(self, mode=True):
        super(BoxModel, self).train(mode=mode)
    ...
    def assert_encoder_is_frozen(self):
        assert self.freeze_enc
    ...
    def __str__(self):
        return "{}. Task: {}. Freeze ENC: {}. ".format(
                self.name, self.task, self.freeze_enc)
...
class DenseBoxModel(torch.nn.Module):
    def initialize(self):
        if self.box_head is not None:
            init.initialize_head(self.box_head)
    ...
    def forward(self, x):
        ...
        box = self.box_head(features[-1])

        self.coords = box.detach()

        return box

    def train(self, mode=True):
        super(DenseBoxModel, self).train(mode=mode)
