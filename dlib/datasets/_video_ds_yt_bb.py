...
def check_csv(path: str):
    ...
    for n in more:
        print(f'videos with {n} classes: {more[n]}')

    return stats


def check_object_loc():
    ds = constants.YTBB
    root = get_root_wsol_dataset()
    root_ds = join(root, ds)
    files = find_files_pattern(root_ds, '*.csv')
...
if __name__ == '__main__':
    ...
    # 0. check whether the datsaet is ok for localization. == no multi class
    # in one video/frame.
